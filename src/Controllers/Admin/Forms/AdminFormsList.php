...
class AdminFormsList extends Controller {
    ...
    public function getRequest() {
        ...
        foreach ( $this->applicationBuilder->getJsonloader()->getResourcesByType( 'form' ) as $res ) {
            ...
            $table->addUnfilteredColumn( Button::get($this->applicationBuilder->getRouterContainer()->makeRelativeUrl( Router::ROUTE_ADMIN_FORM_VIEW, 'res='.$res->name ), 'View', Button::COLOR_GRAY.' '.Button::SMALL ) );
            $table->closeRow();
        }
        $table->closeTBody();
