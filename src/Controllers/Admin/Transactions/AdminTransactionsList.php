...
class AdminTransactionsList extends Controller {
    ...
    public function getRequest() {
        ...
        foreach ( $this->applicationBuilder->getJsonloader()->getResourcesByType( 'transaction' ) as $res ) {
            ...
            $table->addUnfilteredColumn( Button::get($this->applicationBuilder->getRouterContainer()->makeRelativeUrl( ResourceRouter::ROUTE_ADMIN_TRANSACTION_VIEW, 'res='.$res->name ), 'View', Button::COLOR_GRAY.' '.Button::SMALL ) );
            $table->closeRow();
        }
        $table->closeTBody();

        $this->menucontainer    = array( new AdminMenu( $this->applicationBuilder->getSetup()->getAppNameForPageTitle(), ResourceRouter::ROUTE_ADMIN_TRANSACTION_LIST ) );
        $this->leftcontainer    = array( new AdminSidebar( $this->applicationBuilder->getSetup()->getAppNameForPageTitle(), ResourceRouter::ROUTE_ADMIN_TRANSACTION_LIST, $this->applicationBuilder->getRouterContainer() ) );
