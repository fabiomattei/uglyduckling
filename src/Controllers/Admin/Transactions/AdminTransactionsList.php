...
class AdminTransactionsList extends Controller {
    ...
    public function getRequest() {
        ...
        foreach ( $this->jsonloader->getResourcesByType( 'transaction' ) as $res ) {
            ...
            $table->addUnfilteredColumn( Button::get($this->routerContainer->make_url( Router::ROUTE_ADMIN_TRANSACTION_VIEW, 'res='.$res->name ), 'View', Button::COLOR_GRAY.' '.Button::SMALL ) );
            $table->closeRow();
        }
        $table->closeTBody();
